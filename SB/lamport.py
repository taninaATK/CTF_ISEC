...
class LamportPrivateKey(LamportKey):
    ...
    def keygen2(key):
        return LamportPrivateKey(base64.b64decode(key))
...
with open('lamport.json') as f:
    data = json.load(f)

for d in data:
    s = d["signature"]
    m = d["message"].encode()
    bsig = bytes.fromhex(s)
    s = [bsig[i:i+32] for i in range(0, 8192, 32)]
    h = int.from_bytes(sha256(m).digest(), byteorder='big')
    for i in range(256):
        b = (h >> i) & 1
...
key = LamportPrivateKey(key_byte)
msg = b'gelds brews ratio earth tabby'
sig = key.sign(msg)
public_key = key.pk()
